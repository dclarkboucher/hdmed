...
the exposure, conditional $\mathbf{C}$. Our chief interest in performing 
...
more detail.) The others at the very least produce estimates of $\beta_a$,
...
are laid out Song et al. (2019) (see `mediate_bslmm` for complete reference).
...
"high-dimensional" generously, as the dataset has only 20 mediators). Let us
...
the default options, we only need to input our $A$, $\mathbf{M}$, and $Y$.
...
generally useful when $\mathbf{\alpha_a}^T \mathbf{\beta_m}$ and $beta_m$ have
